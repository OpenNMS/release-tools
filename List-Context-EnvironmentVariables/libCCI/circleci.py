...
class circleci:
    ...
    def getContextForOwner(self, owner_slug):
        """https://circleci.com/api/v2/context?owner-slug="""
        _url = (
            self._baseUrl
            + self._baseApiUrl
            + "/v2/context?owner-slug="+owner_slug
            )
        return self._navigateReturnResult(_url).json()

    def getContextEnvironmentVariable(self, context_id):
        """https://circleci.com/api/v2/context/{context-id}/environment-variable"""
        _url = (
            self._baseUrl
            + self._baseApiUrl
            + "/v2/context/"+context_id+"/environment-variable"
            )
        return self._navigateReturnResult(_url).json()
